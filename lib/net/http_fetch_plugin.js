  ...
  static async request_(uri, requestType, init, abortStatus, progressUpdated,
    ...
    try {
      ...
      headersReceived(shaka.net.HttpFetchPlugin.headersToGenericObject_(
          response.headers));
      // LLDIAG: log request/response headers relevant to LL DASH/HLS.
      try {
        const reqId =
            Date.now().toString(36) + '-' +
            Math.floor(Math.random() * 1e6).toString(36);
        const reqHeaders = /** @type {!Headers} */(init.headers);
        const reqRange = reqHeaders && reqHeaders.get ?
            (reqHeaders.get('Range') || reqHeaders.get('range')) : null;
        const cr = response.headers.get('Content-Range');
        const cl = response.headers.get('Content-Length');
        shaka.log.v1('LLDIAG HTTP:', init.method || 'GET', uri,
            'Range=', reqRange || 'n/a',
            'status=', response.status,
            'Content-Range=', cr || 'n/a',
            'Content-Length=', cl || 'n/a',
            'reqId=', reqId);
      } catch (e) {
