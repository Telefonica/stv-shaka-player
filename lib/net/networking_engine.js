// Note: RequestContext typedef is declared in externs; do not redefine here.
/*! @license
 * Shaka Player
...
shaka.net.NetworkingEngine = class extends shaka.util.FakeEventTarget {
  ...
  send_(type, request, context, backoff, index, lastError,
    ...
    // LOG: registrar cada petición de segmento, incluyendo si es reintento
    if (type === shaka.net.NetworkingEngine.RequestType.SEGMENT) {
      const uriLog = request.uris[index];
      let rangeLog = undefined;
      if (request.headers) {
        rangeLog = request.headers['Range'] || request.headers['range'];
      }
      const retryLog = index > 0 ? 'RETRY' : 'FIRST_ATTEMPT';
      let contextId = undefined;
      let reason = retryLog === 'RETRY' ? 'retry' : 'request';
      // Si el contexto tiene información de cambio de duración, lo registramos
      if (context && context.segment &&
      /** @type {?} */ (context).durationChanged) {
        reason = 'duration_changed_after_manifest_update';
      }
      if (context && context.segment) {
        const seg = /** @type {!shaka.media.SegmentReference} */ (
          context.segment
        );
        const start = seg.getStartTime ? seg.getStartTime() : seg.startTime;
        const end = seg.getEndTime ? seg.getEndTime() : seg.endTime;
        contextId = start != null ? start : end;
      }
      const prefix = context && context.stream ?
          '(' + context.stream.type + ':' + context.stream.id + ')' : '';
      if (prefix) {
        shaka.log.v1(prefix, '[SEGMENT_REQUEST]', {
          uri: uriLog,
          range: rangeLog,
          attempt: retryLog,
          contextId: contextId,
