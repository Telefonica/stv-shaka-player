...
shaka.media.StreamingEngine = class {
  ...
  constructor(manifest, playerInterface) {
    ...
    this.fetchSeqByStreamId_ = new Map();
  }
  ...
  async makeAbortDecision_(mediaState) {
    shaka.log.v1(shaka.media.StreamingEngine.logPrefix_(mediaState),
        'LLDIAG makeAbortDecision_ enter');
  ...
  shouldAbortCurrentRequest_(mediaState) {
    ...
    if (timeToFetchNewSegment < safeBufferedAhead) {
      shaka.log.v1(shaka.media.StreamingEngine.logPrefix_(mediaState),
          'LLDIAG abort: true (faster new seg)',
          'timeToFetchNewSegment=', timeToFetchNewSegment,
          'safeBufferedAhead=', safeBufferedAhead);
      return true;
    }
    ...
    if (bytesRemaining > newSegmentSize) {
      shaka.log.v1(shaka.media.StreamingEngine.logPrefix_(mediaState),
          'LLDIAG abort: true (bytesRemaining > newSegSize)',
    ...
    shaka.log.v1(shaka.media.StreamingEngine.logPrefix_(mediaState),
        ...
        'timeToFetchNewSegment=', timeToFetchNewSegment,
        'safeBufferedAhead=', safeBufferedAhead,
        'bytesRemaining=', mediaState.operation.getBytesRemaining(),
        'newSegmentSize=', newSegmentSize);
    return false;
  }
  ...
  forceClearBuffer_(mediaState) {
    ...
    if (mediaState.performingUpdate) {
      ...
      shaka.log.v1(logPrefix, 'LLDIAG set waitingToClearBuffer:',
          'safeMargin=0');
      return;
    }
  ...
  createMediaState_(stream) {
    return /** @type {shaka.media.StreamingEngine.MediaState_} */ ({
      ...
      hasError: false,
      operation: null,
      pendingSegmentStartTime: null,
      lastSegmentWasChunked: false,
    });
  }
  ...
  async onUpdate_(mediaState) {
    ...
    if (mediaState.waitingToClearBuffer) {
      ...
      shaka.log.v1(logPrefix, 'LLDIAG onUpdate_: waitingToClearBuffer path');
  ...
  update_(mediaState) {
    ...
    if (this.shouldSkipSegmentReferenceDueToPending_(mediaState, reference)) {
      shaka.log.v1(logPrefix, 'LLDIAG skip fetch; pending segment start',
          'refStart=', reference.startTime);
      this.scheduleUpdate_(mediaState, 0);
      return null;
    }

    if (this.shouldSkipSegmentReferenceDueToBuffer_(
        mediaState, reference, bufferEnd)) {
      ...
      shaka.log.v1(logPrefix, 'LLDIAG skip fetch; buffer already covers',
          'refStart=', reference.startTime,
          'refEnd=', reference.endTime,
          'bufferEnd=', bufferEnd);
      this.scheduleUpdate_(mediaState, 0);
      return null;
    }
    ...
    if (timeNeeded >= minTimeNeeded + maxRunAhead) {
      ...
      shaka.log.v1(logPrefix, 'LLDIAG throttle run-ahead:',
          'timeNeeded=', timeNeeded,
          'minTimeNeeded=', minTimeNeeded,
          'maxRunAhead=', maxRunAhead);
      return this.config_.updateIntervalSeconds;
    }
  ...
  getSegmentReferenceNeeded_(mediaState, presentationTime, bufferEnd) {
    ...
    if (mediaState.segmentIterator) {
      ...
      // appended.
      const cur = mediaState.segmentIterator.current();
      if (cur) {
        const u = (cur.getUris && cur.getUris()[0]) || 'n/a';
        shaka.log.v1(logPrefix, 'LLDIAG refNeeded (same stream):',
            'ref=', cur.startTime, '-', cur.endTime,
            'uri=', u);
      }
      return cur;
      ...
      if (ref == null) {
        ...
        const u = (ref.getUris && ref.getUris()[0]) || 'n/a';
        shaka.log.v1(logPrefix, 'LLDIAG refNeeded (from other stream):',
            'timeNeeded=', time,
            'ref=', ref.startTime, '-', ref.endTime,
            'uri=', u);
      }
      return ref;
    } else {
  ...
  /**
   * Returns true when |reference| starts where the last appended segment
   * ended and the buffer already covers its advertised end time.
   *
   * @param {!shaka.media.StreamingEngine.MediaState_} mediaState
   ...
   * @private
   */
  shouldSkipSegmentReferenceDueToBuffer_(mediaState, reference, bufferEnd) {
    if (!bufferEnd || !mediaState.lastSegmentReference || !reference) {
      return false;
    }

    const last = mediaState.lastSegmentReference;
    if (reference.startTime != last.startTime) {
      return false;
    }

    if (reference.endTime <= last.endTime) {
      return false;
    }

    if (mediaState.lastSegmentWasChunked) {
      return true;
    }

    return bufferEnd >= reference.endTime;
  }

  /**
   * Returns true when the reference has the same startTime as the segment
   ...
   * @private
   */
  shouldSkipSegmentReferenceDueToPending_(mediaState, reference) {
    const pending = mediaState.pendingSegmentStartTime;
    if (pending == null || !reference) {
      return false;
    }
    return reference.startTime == pending;
  }
  ...
  async fetchAndAppend_(mediaState, presentationTime, reference) {
    ...
    const uri0 = (reference.getUris && reference.getUris()[0]) || 'n/a';
    shaka.log.v1(logPrefix,
        ...
        'uri=' + uri0);
    shaka.log.v1(logPrefix, 'LLDIAG select ref:',
        'uri=', uri0,
        'time=', reference.startTime, '-', reference.endTime,
    ...
    mediaState.pendingSegmentStartTime = reference.startTime;
    mediaState.lastSegmentWasChunked = false;
    ...
    try {
      ...
      if (this.config_.lowLatencyMode && isReadableStreamSupported && isMP4 &&
        ...
        mediaState.lastSegmentWasChunked = true;
        const streamDataCallback = async (data) => {
          ...
          try {
            shaka.log.v2(logPrefix,
                'LLDIAG chunked: received data for', uri0,
            ...
            if (sawMDAT) {
              ...
              shaka.log.v2(logPrefix,
                  'LLDIAG chunked: append MDAT part for', uri0,
                  'bytes=', dataToAppend.byteLength,
                  'remaining=', remaining.byteLength);
              await this.append_(
        ...
        if (!callbackCalled) {
          ...
          if (mediaState.waitingToClearBuffer) {
            ...
            shaka.log.v1(
                logPrefix,
                'LLDIAG skip append ' +
                '(waitingToClearBuffer) for',
                uri0,
                'ref=', reference.startTime, '-', reference.endTime);
      ...
      try {
        const u0 = (reference.getUris && reference.getUris()[0]) || 'n/a';
        shaka.log.v1(logPrefix, 'LLDIAG appended:',
            'uri=', u0,
            'time=', reference.startTime, '-', reference.endTime);
      } catch (e) {}
      ...
      mediaState.pendingSegmentStartTime = null;
    }
  }
  ...
  async fetch_(mediaState, reference, streamDataCallback) {
    ...
    let op = null;
    const diagUri0 = (reference.getUris && reference.getUris()[0]) || 'n/a';
    const isSegRef = reference instanceof shaka.media.SegmentReference;
    let diagStart = null;
    let diagEnd = null;
    if (isSegRef) {
      // Use methods to keep Closure happy with union types.
      diagStart = /** @type {!shaka.media.SegmentReference} */(reference)
          .getStartTime();
      diagEnd = /** @type {!shaka.media.SegmentReference} */(reference)
          .getEndTime();
    }
    if (mediaState.segmentPrefetch) {
      ...
      if (op) {
        shaka.log.v1('LLDIAG fetch_: using PREFETCH for', diagUri0,
            'time=', isSegRef ? (diagStart + '-' + diagEnd) : 'init');
      } else {
        shaka.log.v2('LLDIAG fetch_: prefetch miss for', diagUri0,
            'time=', isSegRef ? (diagStart + '-' + diagEnd) : 'init');
      }
    }
    if (!op) {
      shaka.log.v1('LLDIAG fetch_: NETWORK fetch for', diagUri0,
          'time=', isSegRef ? (diagStart + '-' + diagEnd) : 'init');
  ...
  dispatchFetch_(reference, stream, streamDataCallback) {
    // Build a per-stream monotonically increasing sequence number to
    // correlate requests in logs.
    const prev = this.fetchSeqByStreamId_.get(stream.id) || 0;
    const seq = prev + 1;
    this.fetchSeqByStreamId_.set(stream.id, seq);
    ...
    const diagUri0 = (reference.getUris && reference.getUris()[0]) || 'n/a';
    const seg = segment;
    shaka.log.v1('LLDIAG dispatchFetch_:',
        'uri=', diagUri0,
        'range=', reference.startByte + '-' + reference.endByte,
        'time=', (seg && seg.startTime), '-', (seg && seg.endTime),
        'type=', stream.type,
        'id=', stream.id,
        'seq=', seq);
  ...
  async clearBuffer_(mediaState, flush, safeMargin) {
    ...
    shaka.log.v1(logPrefix, 'LLDIAG clearBuffer_ start:',
        'flush=', !!flush,
    ...
    shaka.log.v1(logPrefix, 'LLDIAG clearBuffer_ done');
 ...
 *   pendingSegmentStartTime: ?number,
 *   lastSegmentWasChunked: boolean,
 ...
 * @property {?number} pendingSegmentStartTime
 *   Start time of the SegmentReference currently being fetched.
