...
shaka.media.MediaSourceEngine = class {
  ...
  constructor(video, textDisplayer, onMetadata, lcevcDec) {
    ...
    /**
     * Tracks last seen manifest segments per content type for instrumentation.
     * @private {!Object<string, !Array<{startTime:number, endTime:number}>>}
     */
    this._lastManifestSegments = {};
  }
  ...
  async init(streamsByType, sequenceMode=false,
    ...
    // Instrumentation: log manifest update and segment end-time changes.
    shaka.log.v1('[MANIFEST_UPDATE]', {
      manifestType: this.manifestType_,
      ignoreManifestTimestampsInSegmentsMode:
        this.ignoreManifestTimestampsInSegmentsMode_,
      time: Date.now(),
      streams: Array.from(streamsByType.keys()).map((k) => {
        const s = streamsByType.get(k);
        return {
          type: k,
          id: s.id,
          segmentIndex: s.segmentIndex ? 'present' : 'n/a',
        };
      }),
    });

    this._lastManifestSegments = this._lastManifestSegments || {};
    const manifestChanges = [];
    for (const k of Array.from(streamsByType.keys())) {
      const s = streamsByType.get(k);
      if (s && s.segmentIndex &&
          typeof s.segmentIndex.getReferences === 'function') {
        const refs = s.segmentIndex.getReferences();
        const prevRefs = this._lastManifestSegments[k] || [];
        refs.forEach((ref, idx) => {
          const prev = prevRefs[idx];
          if (prev && ref && ref.startTime === prev.startTime &&
              ref.endTime !== prev.endTime) {
            manifestChanges.push({
              ...
              newEndTime: ref.endTime,
            });
          }
        });
        this._lastManifestSegments[k] = refs.map((r) => ({
    ...
    if (manifestChanges.length > 0) {
      shaka.log.v1('[MANIFEST_SEGMENT_CHANGE]', {
  ...
  async appendBuffer(
    ...
    if (goog.DEBUG && reference && !reference.isPreload() && !isChunkedData) {
      ...
      if (newBuffered) {
        // LLDIAG: concise post-append trace to aid LL investigations.
        try {
          const uri0 = (reference.getUris && reference.getUris()[0]) || 'n/a';
          shaka.log.v1('LLDIAG MSE append:', contentType,
              'ref=', reference.startTime, '-', reference.endTime,
              'uri=', uri0,
              'added=', {start: newBuffered.start, end: newBuffered.end});
        } catch (e) {
