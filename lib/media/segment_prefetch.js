...
shaka.media.SegmentPrefetch = class {
  ...
  prefetchSegments(startReference, skipFirst = false) {
    ...
    while (
      this.segmentPrefetchMap_.size < this.prefetchLimit_ &&
      reference != null
    ) {
      ...
      if (prefetchAllowed && !this.segmentPrefetchMap_.has(reference)) {
        ...
        shaka.log.v1(logPrefix, '[PREFETCH_SEGMENT]', {
      ...
      } else if (!prefetchAllowed) {
        shaka.log.v1(logPrefix, '[PREFETCH_SEGMENT_SKIP]', {
          uri: reference.getUris()[0],
          startTime: reference.startTime,
          endTime: reference.endTime,
          reason: reason,
      ...
      } else if (this.segmentPrefetchMap_.has(reference)) {
        shaka.log.v1(logPrefix, '[PREFETCH_SEGMENT_DUPLICATE]', {
          uri: reference.getUris()[0],
          startTime: reference.startTime,
          endTime: reference.endTime,
